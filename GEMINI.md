...
- **`gaze-gui`**: A GTK4/Adwaita application for user-friendly face enrollment and management.
- **`gaze-core`**: Shared logic for camera interaction, capture sessions, face checking, configuration, and DBus communication.

## Architecture & Data Flow

1. **Capture**: Frames are captured via OpenCV (`gaze-core/src/camera.rs`).
...
- **Build PAM module**: `cargo build -p pam-gaze --release` (produces `libpam_gaze.so`)
- **Build GUI**: `cargo build -p gaze-gui --release`
...
- **Safety**: `pam-gaze` uses unsafe C FFI. Any changes there must be rigorously audited for memory safety.
